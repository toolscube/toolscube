...
function extractMeta(html: string, baseUrl: string): Extracted {
  ...
  const description = meta['og:description']?.[0] ?? meta['twitter:description']?.[0] ?? meta['description']?.[0] ?? '';
...
export async function GET(req: Request) {
  try {
    ...
    const res = await fetch(normalized, {
      method: 'GET',
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; ToolsHub-OGPreview/1.0; +https://example.com/bot) AppleWebKit/537.36',
    ...
    return Response.json({ ok: false, error: msg }, { status: 500, headers: { 'Cache-Control': 'no-store' } });
  }
}
