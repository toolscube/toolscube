...
function estimateCharset(pw: string) {
  ...
  if (hasUpper) size += 26;
  if (hasDigit) size += 10;
  if (symbolMatch) size += 33; // conservative ASCII punctuation count
  if (hasSpace) size += 1;

  if (!asciiOnly) {
    // add an estimate for non-ASCII characters actually used
    const extras = new Set([...pw].filter((ch) => !/[A-Za-z0-9\s!@#$%^&*()\-\_=+\[\]{};:'",.<>/?`~\\|]/.test(ch)));
    size += Math.max(1, extras.size); // at least +1
  }
...
function bandFromEntropy(bits: number): StrengthBand {
  if (bits < 28) return 'Very Weak'; // trivial
  if (bits < 36) return 'Weak'; // easy
  if (bits < 60) return 'Fair'; // reasonable
  if (bits < 128) return 'Strong'; // solid
...
const RATES = {
  online_throttled: 10, // 10 guesses/sec
  online_fast: 100, // 100/sec
  offline_slow_hash: 1e5, // 100k/sec (argon/bcrypt with cost)
  offline_fast_hash: 1e10, // 10B/sec (GPU, weak hash)
  nation_state: 1e12, // 1T/sec (very optimistic upper bound)
};
