...
export default function VatCalculatorPage() {
  ...
  function exportCSV() {
    const rows: string[][] = [
      ...
      [mode === 'add' ? 'Add Tax' : 'Remove Tax', numStr(price), String(rate), numStr(calc.net), numStr(calc.tax), numStr(calc.gross)],
    ];
      ...
      <MotionGlassCard>
        ...
        <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Add tax to a net price or remove tax from a gross price.</p>
          </div>
        ...
        <GlassCard className="shadow-sm">
          <CardHeader>
            ...
            <CardDescription>Choose mode, set price and tax rate. Use quick chips for common rates.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Add = price is net (pre‑tax). Remove = price is gross (tax‑inclusive).</p>
            </div>

            <div className="space-y-2">
              <Label htmlFor="price">{mode === 'add' ? 'Net Price' : 'Gross Price'}</Label>
              <Input id="price" inputMode="decimal" value={numDisplay(price)} onChange={(e) => setPrice(safeNum(e.target.value))} />
              <p className="text-xs text-muted-foreground">Enter the {mode === 'add' ? 'pre‑tax' : 'tax‑inclusive'} amount.</p>
            </div>
        ...
        <GlassCard className="shadow-sm">
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-4 sm:grid-cols-3">
              <ResultBox label="Net" value={formatMoney(calc.net, currency, round2)} onCopy={() => copy(calc.net, 'net')} copied={copied === 'net'} />
              <ResultBox label="Tax" value={formatMoney(calc.tax, currency, round2)} onCopy={() => copy(calc.tax, 'tax')} copied={copied === 'tax'} />
              <ResultBox label="Gross" value={formatMoney(calc.gross, currency, round2)} onCopy={() => copy(calc.gross, 'gross')} copied={copied === 'gross'} />
            </div>
            ...
            <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Add Tax: gross = net × (1 + r). Remove Tax: net = gross ÷ (1 + r). Tax = gross − net.</span>
            </div>
