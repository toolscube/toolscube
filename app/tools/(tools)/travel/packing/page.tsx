  ...
  async function copyText() {
    const lines = [
      ...
      ...filtered.flatMap((g) => [
        ...
        ...g.items.map(
          (i) =>
            `${i.checked ? "[x]" : "[ ]"} ${i.label}${i.qty ? ` ×${i.qty}` : ""}${i.must ? " ★" : ""}${i.note ? ` — ${i.note}` : ""}`,
        ),
      ]),
    ].join("\n");
    try {
        ...
        <GlassCard className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-6 py-5">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Template → tune → check off. Everything saves in your browser.
            </p>
          </div>
        ...
        <GlassCard className="px-6 py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex items-center gap-2">
            <Input
              ...
              className="w-full md:w-72 bg-background/60 backdrop-blur"
            />
          </div>
        </GlassCard>
